...
export default function AdvancedTicketList() {
  ...
  const { hotels } = useTickets(false); // Get all hotels for filter dropdown
  ...
  const apiFilters = {
    ...
    hotel_id: filters.hotel_id ? parseInt(filters.hotel_id) : undefined,
  ...
  const { tickets, total, isLoading, error } = useTickets(apiFilters);
