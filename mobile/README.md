...
- Secure bearer token persistence with Expo Secure Store.
...
- Map-first citizen reporting with `GET /api/containers` search, optional GPS centering on the citizen position, nearby shortcuts plus edge-arrow indicators for the nearest 3-5 containers, highlighted nearby-container markers limited to that same nearest set, a bottom-sheet composer with location refresh and optional camera evidence, 1-hour duplicate protection from the backend, zone-manager notification queuing, and post-submit history/challenge refresh.
...
- Citizen pages use a shared header shell with centered EcoTrack mark, authenticated profile avatar trigger, swipe-open sidebar navigation, and a tappable logo that returns users to their home route.
